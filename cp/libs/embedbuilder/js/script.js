/**
 * Discord Embed Builder by Glitchii,
 * using a modified version capable of storing and transfering data by MyT from No Name Studio.
 */

window.options ??= {};
window.inIframe ??= top !== self;
mainHost = "cp.nnsvn.me";
...
addEventListener('DOMContentLoaded', () => {
    ...
    //         })
    //         // split the text into lines and then wrap each line in a span tag, if there is no text in the line, add a <br /> tag to make a break line.
    //         .split('\n').map(line => {
...
function saveJson() {
    ...
    localStorage.setItem('embedbuilderjson', jsonStr);
}
